...
export function point_circle(px, py, cx, cy, r) {
    return distance(px, py, cx, cy) <= r;
}

export function line_point(x1, y1, x2, y2, px, py) {
...
export function line_circle(x1, y1, x2, y2, cx, cy, r) {
    const lineLength = distance(x1, y1, x2, y2);
    ...
    const onSegment = line_point(x1, y1, x2, y2, px, py);
    if (!onSegment) {
        return false;
    }
